...
update_list(){
	...
	wget -P /tmp --no-check-certificate $china_domain_list
	...
	wget -O- --no-check-certificate $china_routing_list > /tmp/Routing_IPv4_orign.txt
	[ "$?" != "0" ] && echo_date 下载错误，退出！ && exit 0
...
update_list > /tmp/upload/ss_log.txt 2>&1
echo XU6J03M6 >> /tmp/upload/ss_log.txt 2>&1
sleep 1
http_response "$1"