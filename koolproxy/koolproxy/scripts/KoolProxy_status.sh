...
if [ "$status" == "2" ];then
	http_response "【$date】 KoolProxy $version  进程运行正常！(PID: $pid) @@静态规则：$rules_date_local / $rules_nu_local条&nbsp;&nbsp;&nbsp;&nbsp;视频规则：$video_date_local&nbsp;&nbsp;&nbsp;&nbsp;每日/自定义规则：$daily_nu_local条 / $custom_nu_local条"
else
