...
function getLocalApp(obj){
	...
	for(var p in obj) {
		...
		if(p.indexOf("name") > 0 ){  
			...
			vhtml1 += '<div class="apps" style="background:url('+bgimg+');" onmouseover="change1(this);" onmouseout="change2(this);"><a href="'+aurl+'" title="'+description+'"><div class="infos"><img class="appimg" src="'+appimg+'"/></div><div class="app-name"><p>'+name+'</p><p class="desc">'+description+'</p></div></a><div class="appDesc">'+appButton+'</div></div>';
		}  						
	}
	$("#app1-server1-basic-tab").html('<i class="icon-system"></i> 已安装（'+j+'）');
	$(".tabContent1").html(vhtml1);
